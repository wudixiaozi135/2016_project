 ...
 */
class ColorPicker extends egret.Sprite
{
	private _chooseColorHandler:(color:number)=>void;

	private _colorArr:string[];
	private _moveGrid:egret.Shape;
	private _gridSize:number = 11;
	private _rowNum:number = 12;
	private _columnNum:number = 18;

	private _canvas:egret.Sprite;
	private _canvasInitX:number = 5;
	private _canvasInitY:number = 30;

	private _txtColor:egret.TextField;
	private _colorIndex:number = 0;
	private _isClickSamePoint:boolean = true;
	...
	private initUI():void
	{
		...
		var txtBg:egret.Shape = new egret.Shape();
		...
		var size:number = this._rowNum * this._columnNum;
	...
	public setChooseColor(handler:(color:number)=>void):ColorPicker
	{
	...
	private limitMove(moveX:number, moveY:number):void
	{
		var row:number = Math.floor(moveY / this._gridSize);
		var column:number = Math.floor(moveX / this._gridSize);

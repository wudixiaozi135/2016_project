...
var GamePreView = (function (_super) {
    ...
    p.onAddToStage = function (evt) {
        ...
        this.addChild(bg);
        var preDes = this.createBitmapByName("tips");
        ...
        var logo = this.createBitmapByName("title_popstar");
        this.addChild(logo);
        ...
        var maxScoreTF = new egret.TextField();
        maxScoreTF.size = 20;
        ...
        maxScoreTF.y = 15;
        var liNum = egret.localStorage.getItem("toolStar");
        if (liNum == null) {
            //首次赠送刀具
            var rewardView = new GameFirstRewardView();
            this.addChild(rewardView);
            rewardView.y = -rewardView.height;
            egret.Tween.get(rewardView).to({ y: 0 }, 300);
        }
    };
    p.onStartNewGame = function (evt) {
        if (egret.localStorage.getItem("hasgame")) {
    ...
    p.onClearGameInfo = function (evt) {
    ...
    p.onShopWindow = function (evt) {
        GameShopView.getInstance().onOpenShopWindow(this);
    };
    p.onRefreshView = function () {
        if (egret.localStorage.getItem("hasgame")) {
            this.rePlayBtn.onSetEnable(true);
            this.rePlayBtn.addEventListener(egret.TouchEvent.TOUCH_TAP, this.onStartGame, this);
        }
